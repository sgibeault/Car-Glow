...
class InsideLights : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        createNewPattern.setOnClickListener {
            val intent = Intent(this, InsideLightsEditor::class.java).apply {}
    ...
    override fun onResume() {
        ...
        Log.i("InsideLights", "Preference count: $preferenceCount")
        ...
        listView.onItemClickListener = AdapterView.OnItemClickListener { _, _, position, _ ->
            Log.i("InsideLights", "Tapped position: $position")
            val position1 = position + 1
            preferenceItem = preferences.getString("Inside Entry $position1: Color Pattern", null)
            if (preferenceItem != null) {
                mainActivity.sendCommand(preferenceItem!!)
            } else {
                Log.i("InsideLights", "No saved data at position $position1")
            }
            preferenceItem = preferences.getString("Inside Entry $position1: Light Pattern", null)
            if (preferenceItem != null) {
                mainActivity.sendCommand(preferenceItem!!)
            } else {
                Log.i("InsideLights", "No saved data at position $position1")
            }
            preferenceItem = preferences.getString("Inside Entry $position1: Brightness", null)
            if (preferenceItem != null) {
                mainActivity.sendCommand(preferenceItem!!)
            } else {
                Log.i("InsideLights", "No saved data at position $position1")
