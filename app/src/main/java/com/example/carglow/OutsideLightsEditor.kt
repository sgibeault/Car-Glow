...
class OutsideLightsEditor : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.outside_lights_editor)
        ...
        sbR.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
                ...
                if (selectedLed != 0 && elapsedTime > updateCooldown) {
                    sendCounter += 1
                    Log.i("OutsideLightsEditor", "Send counter updated: $sendCounter times")
        ...
        saveButton.setOnClickListener{
            ...
            editor.putString("Outside Entry ${insidePreferenceCounter+1}: Name", "Custom pattern ${insidePreferenceCounter}")
            numColors = led1IsActive + led2IsActive + led3IsActive + led4IsActive + led5IsActive
            numColorsString = numColors.toString()
            sendData = "m" + numColorsString + "x" +
                    led1ColorR + "x" + led1ColorG + "x" + led1ColorB + "x" +
                    ...
                    led5ColorR + "x" + led5ColorG + "x" + led5ColorB + "x" +
                    inOrOut + "x"
            editor.putString("Outside Entry ${insidePreferenceCounter+1}: Color Pattern", sendData)
            sendData = "l" + (selectedLightPattern+1).toString() + "x" + inOrOut + "x"
            editor.putString("Outside Entry ${insidePreferenceCounter+1}: Light Pattern", sendData)
            sendData = "n" + "ff" + "x" + inOrOut + "x"
            editor.putString("Outside Entry ${insidePreferenceCounter+1}: Brightness", sendData)
            editor.apply()

            numberOfPreferences = preferences.all.size
            Log.i("OutsideLightsEditor", "Number of shared preferences entries: $numberOfPreferences")
            super.onBackPressed()
        }
    }
}